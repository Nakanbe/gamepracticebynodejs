	...
	analypage(function(GameData){
		...
		re_query(j, GameData, function(){
			if(flag == 1){  //是否需要INSERT
				...
				file.writeFile("msg3.txt", queryinsert);
...
function re_query(j, GameData, callback){
	console.log(j);
	var selectquery = "SELECT " + "bet_odds0 as odds0, " +"bet_odds1 as odds1, " +"bet_odds2 as odds2, " +"bet_odds3 as odds3, " +"bet_odds4 as odds4, " +"bet_odds5 as odds5, " +"bet_num as num, " + "bet_ID " + "FROM " +"betgame " + "WHERE " +"bet_date = '" + GameData[j]['date'] + "' AND " +"bet_leaguname = '" + GameData[j]['leaguname'] + "' AND " +"bet_ht = '" + GameData[j]['ht'] + "' AND " +"bet_at = '" + GameData[j]['at'] + "' AND " +"bet_time = '" + GameData[j]['time'] + "'";
	// console.log(selectquery);
	connection.query(selectquery, function(error, result){  //j表示第幾筆資料  result為query結果
	  console.log(j+ " : IN LOOP");
		if(error) throw error;
		if(result == ""){  //先確認有無資料
			//沒有就INSERT
			console.log("INSERT");
