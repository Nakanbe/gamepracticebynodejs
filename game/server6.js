...
server_io.sockets.on('connection', function(socket){
	socket.on('analy', function(){  //收到analy就開始分析網頁
		checksql(connection, function(){
			var resultarr = insertGameJSON.concat(updateGameJSON);  //合併新增和更新的資料
			var resultJSON = {};
			...
			insertGameJSON = [];
			updateGameJSON = [];
			queryinsert = "INSERT INTO betgame (bet_date, bet_leaguname, bet_ht, bet_at, bet_time, bet_rq0, bet_rq1, bet_odds0, bet_odds1, bet_odds2, bet_odds3, bet_odds4, bet_odds5, bet_num) VALUES ";
		});
	});
	//搜尋
...
function checksql(connection, callback){
	analypage(function(GameData){
		...
		if(GameData== 'no_data'){  //網頁沒有資料
			callback();
		}
		else{
			re_query(j, GameData, function(){
				if(flag == 1){  //是否需要INSERT
					queryinsert = queryinsert.substring(0, queryinsert.length-1);  //跟DB INSERT, 去掉最後的逗號
					flag = 0;
...
function re_query(j, GameData, callback){
	var selectquery = "SELECT " + 
										...
										"FROM " +
											"betgame " + 
										"WHERE " +
											"bet_date = '" + GameData[j]['date'] + "' AND " +
											"bet_leaguname = '" + GameData[j]['leaguname'] + "' AND " + 
											"bet_ht = '" + GameData[j]['ht'] + "' AND " +
											"bet_at = '" + GameData[j]['at'] + "' AND " +
											"bet_time = '" + GameData[j]['time'] + "'";
	connection.query(selectquery, function(error, result){  //j表示第幾筆資料  result為query結果
		if(error) throw error;
		if(result == ""){  //先確認有無資料
			//沒有就INSERT
			queryinsert +=  "('" + GameData[j]['date'] + "','" + 
														 GameData[j]['leaguname'] + "','" + 
														 GameData[j]['ht'] + "','" + 
														 GameData[j]['at'] + "','" +  
														 GameData[j]['time'] + "'," + 
														 GameData[j]['rq0'] + "," +
